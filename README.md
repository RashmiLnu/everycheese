...
```bash
conda create python=3.8 -n cookiecutter
```
...
If for some reason you get an error similar to bellow, is because the DATABASE_URL is configured to `postgres:///cookiecutter` and because of it the generated `DATABASES` settings are configured to connect on PostgreSQL using the socket mode.
...
LINE 1: CREATE DATABASE cookiecutter WITH ENCODING 'UTF-8' OWNER "";
                                                             ^
ERROR:  syntax error at or near ";"
LINE 1: GRANT ALL PRIVILEGES ON DATABASE cookiecutter TO ;
```
...
ERROR:  database "cookiecutter" already exists
GRANT
```
...
```bash
sudo -u postgres dropdb -U postgres --if-exists cookiecutter
